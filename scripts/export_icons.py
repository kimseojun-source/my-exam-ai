...
def save_png(image,path):
 options={'format':'PNG','compress_level':9}
 if image.mode!='RGBA':options['optimize']=True
 image.save(path,**options)
icons=ROOT/'static'/'icons';icons.mkdir(parents=True,exist_ok=True)
for size in (192,512):save_png(scaled(size),icons/f'icon-{size}.png')
save_png(scaled(180),icons/'apple-touch-icon.png')
# Separate maskable asset leaves the artwork within the central safe circle.
mask=Image.new('RGB',(512,512),'#d6ff00');small=scaled(420);mask.paste(small,(46,46));save_png(mask,icons/'maskable-512.png')
res=ROOT/'android'/'res'
for density,size in [('mdpi',48),('hdpi',72),('xhdpi',96),('xxhdpi',144),('xxxhdpi',192)]:
 dest=res/f'mipmap-{density}';dest.mkdir(parents=True,exist_ok=True)
 for name in ['ic_launcher','ic_launcher_round']:save_png(scaled(size),dest/(name+'.png'))
dest=res/'drawable-nodpi';dest.mkdir(parents=True,exist_ok=True)
foreground=Image.new('RGBA',(432,432),(0,0,0,0));size=336
foreground.alpha_composite(original.resize((size,size),Image.Resampling.LANCZOS),((432-size)//2,(432-size)//2));save_png(foreground,dest/'ic_launcher_foreground.png')
...
save_png(scaled(1024),ios/'AppIcon.png');(ios/'Contents.json').write_text(json.dumps({'images':[{'filename':'AppIcon.png','idiom':'universal','platform':'ios','size':'1024x1024'}],'info':{'author':'xcode','version':1}},indent=2))
